...
- **Шаринг слота** (иконка «Поделиться» на [SCR-003](#scr-003--карточка-слота)): решение
  RR-D06 (Phase 2/декор) пересмотрено — функция реализована в MVP как **текстовый шаринг**
  слота (маршрут, дата/время, цена за место, место встречи) через системный диалог
  «Поделиться»; ссылка на слот не прикладывается (deep link/universal link не реализован).
  Доступно **только на Android** (`Intent.ACTION_SEND`); на iOS и Web иконка присутствует, но
  неактивна (см. §7). Детали — `02-development/FEATURE_share-slot.md`.
...
| **Шаринг слота на iOS/Web** — Phase 2 | Текстовый шаринг слота на [SCR-003](#scr-003--карточка-слота) реализован в MVP только на Android (см. §6); распространение на iOS и Web отложено до отдельного решения. |
